...
func (r *requestRepository) Create(c context.Context, req *models.Request) error {
	query := `
        INSERT INTO "request" (name, service_id, owner_id, employee_id, priority, "desc", status)
...
func (r *requestRepository) Get(c context.Context, req *[]models.Request) error {
	query := `SELECT * FROM "request"` //пока оставлю так,сложный запрос выходит
	err := r.conn.SelectContext(c, req, query)
	if err != nil {
		return err
	}
	return nil
...
func (r *requestRepository) GetWithFilter(ctx context.Context, req *[]models.Request, filter models.Request) error {
	...
	// поле "price" в модели Request сейчас нет
